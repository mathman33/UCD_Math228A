...
def RHS(x,y,z):
    return -np.exp(-(x - 0.25)**2 - (y - 0.6)**2 - (z - 0.5)**2)

def do_direct(h,ARGS):
    N = 1/h + 1
    unit = np.linspace(0,1,N,endpoint=True)
    xx,yy,zz = np.meshgrid(unit,unit,unit,sparse=True)
    f = RHS(xx,yy,zz)
    f = f.flatten()
    ...
    A = np.vstack((off_diag,off_diag,off_diag,diag,off_diag,off_diag,off_diag))
    A = scipy.sparse.dia_matrix((A,[-N**2,-N,-1,0,1,N,N**2]),shape=(N**3,N**3))
...
def main():
    ...
    if ARGS.jacobi:
        ...
        with open("jacobi_data_3b.p", "wb") as handle:
    ...
    if ARGS.gauss_seidel:
        ...
        with open("gs_data_3b.p", "wb") as handle:
    ...
    if ARGS.sor:
        ...
        with open("sor_data_3b.p", "wb") as handle:
    ...
    if ARGS.direct:
        ...
        with open("direct_data_3b.p", "wb") as handle:
